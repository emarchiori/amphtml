...
import {createExampleDocument} from './example-page';
...
env => {
  ...
  beforeEach(() => {
    ...
    historyMock = sandbox.mock(win.history);
  ...
  it('fetches the next document within 3 viewports away', function* () {
    xhrMock.expects('fetchDocument')
        .returns(Promise.resolve(doc.createElement('div')));
  ...
  it('updates title/URL when the next document comes into view', function*() {
    xhrMock.expects('fetchDocument')
        .returns(Promise.resolve(createExampleDocument(doc, 'Title 1')));
    historyMock.expects('replaceState');
